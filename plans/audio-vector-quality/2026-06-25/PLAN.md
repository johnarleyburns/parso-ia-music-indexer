...
- [x] `--sample-strategy=midpoint` implemented with size-aware `Range` + short-file fallback; sampling strategy recorded; re-embed is incremental. Unit tests pass.
- [x] `make export-ios` produces + validates the iOS model/tokenizer into `Acalum/Resources/`; app parity test passes against the emitted `test_vectors.json`.
- [x] `make eval-retrieval` prints top-k audio neighbors for the canonical + user prompts; before/after sampling captured.
- [x] (Optional) coverage diagnostic reported (via headless stats `lexical_coverage`).
- [x] `make ci` green; plan doc updated with results.

## Implementation Results (2026-06-25)

### Files Changed
- `internal/config/config.go` — added `--sample-strategy` (default `midpoint`) and `--sample-skip-seconds` (default 20)
- `internal/audio/stream.go` — added `StreamAudioMidpoint()`, `StreamAudioFromURLWithRange()`, `computeMidpointStartByte()`, `getFileSize()`
- `internal/db/db.go` — `sample_strategy TEXT` column on `track_embeddings` in CREATE TABLE + migration
- `internal/db/embeddings.go` — `SaveEmbeddingWithStrategy()`, `GetEmbeddingStrategyCounts()`, `ResetTracksWithStrategy()`, `GetLexicalCoverage()`
- `cmd/tui/main.go` — `analyzeTrack()` wired to dispatch on `cfg.SampleStrategy`; headless stats now include `embedding_strategies` and `lexical_coverage`

### Files Created
- `python_sidecar/eval_retrieval.py` — cross-modal retrieval eval harness
- `plans/audio-vector-quality/2026-06-25/PLAN.md` — this file (copied from IA_INDEXER_AUDIO_QUALITY_PLAN.md)
- `plans/audio-vector-quality/2026-06-25/current_state.md` — implementation status tracker

### Makefile Targets Added
- `make export-ios` — exports CLAP text encoder + tokenizer to sibling iOS app
- `make eval-retrieval` — runs retrieval evaluation (requires sidecar running)

### Stretch Goals Not Implemented
- Multiwindow averaging (`--sample-strategy=multiwindow`) — deferred as stretch goal

### Verification
- `make ci` green: go build + go vet + go test -race -count=1 all pass (12 packages)
